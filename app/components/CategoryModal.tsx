...
export default function CategoryModal({ initial, onSave, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-end justify-center p-4 sm:items-center">
      <div className="w-full max-w-sm bg-gray-100 dark:bg-gray-900 rounded-2xl border border-gray-300 dark:border-gray-700 p-5">
        ...
        {/* Group selector */}
        <div className="mb-4">
          <div className="text-gray-600 dark:text-gray-400 text-xs mb-2">Группа</div>
            ...
            {groupLabels.map(g => (
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg border text-xs font-medium transition-all ${
                  group === g.key ? g.color : 'border-gray-300 dark:border-gray-700 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'
                }`}
              >
                {g.label}
              </button>
        ...
        {/* Name */}
        <div className="mb-4">
          <div className="text-gray-600 dark:text-gray-400 text-xs mb-2">Название</div>
          <input
            type="text"
            value={name}
            onChange={e => { setName(e.target.value); setError('') }}
            placeholder="Например: Такси"
            className="w-full bg-gray-200 dark:bg-gray-800 text-gray-900 dark:text-white p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 placeholder-gray-400 dark:placeholder-gray-600 text-sm"
            autoFocus
          />
        </div>
        ...
        {/* Icon */}
        <div className="mb-4">
          <div className="text-gray-600 dark:text-gray-400 text-xs mb-2">Иконка — выберите или введите эмодзи</div>
            ...
            {SUGGESTED_ICONS.map(em => (
              <button
                ...
                className={`w-9 h-9 rounded-lg text-lg flex items-center justify-center transition-all ${
                  icon === em && !customIcon ? 'bg-green-700 ring-2 ring-green-500' : 'bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700'
                }`}
              >
                {em}
              </button>
          ...
          <input
            ...
            className="w-full bg-gray-200 dark:bg-gray-800 text-gray-900 dark:text-white p-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 placeholder-gray-400 dark:placeholder-gray-600 text-sm"
          />
        </div>

        {/* Preview */}
        <div className="bg-gray-200 dark:bg-gray-800 rounded-xl p-3 flex items-center gap-3 mb-4">
          <span className="text-2xl">{customIcon || icon}</span>
          <span className="text-gray-900 dark:text-white text-sm">{name || 'Название категории'}</span>
        </div>
        ...
        <div className="flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 py-3 rounded-xl bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm transition-colors"
          >
            Отмена
          </button>
          <button
